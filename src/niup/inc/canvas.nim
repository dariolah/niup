...
## constructors
##
##    proc Canvas(action: string): Canvas_t
##
##    proc Canvas(): Canvas_t
##
...
proc `bgcolor=`*(ih: Canvas_t, value: string) {.cdecl.} =
    ## BGCOLOR: Background color. The background is painted only if
    ## the ACTION callback is not defined. If the callback is defined
    ## the application must draw all the canvas contents. In GTK or
    ## Motif if you set the ACTION callback after map then you should
    ## also set BGCOLOR to any value just after setting the callback
    ## or the first redraw will be lost. Default: "255 255 255".
...
proc `dx=`*(ih: Canvas_t, value: string) {.cdecl.} =
    ...
    ## scrollbar. Default: "0.0". XMIN: Minimum value of the
    ## horizontal scrollbar. Default: "0.0". XMAX: Maximum value of
    ## the horizontal scrollbar. Default: "1.0". YMIN: Minimum value
    ## of the vertical scrollbar. Default: "0.0". YMAX: Maximum value
    ## of the vertical scrollbar. Default: "1.0". LINEX: The amount
    ## the thumb moves when an horizontal step is performed. Default:
    ## 1/10th of DX. (since 3.0) LINEY: The amount the thumb moves
    ## when a vertical step is performed. Default: 1/10th of DY.
    ## (since 3.0) XAUTOHIDE: When enabled, if DX >= XMAX-XMIN then
    ## the horizontal scrollbar is hidden. Default: "YES". (since 3.0)
    ## YAUTOHIDE: When enabled, if DY >= YMAX-YMIN then the vertical
    ## scrollbar is hidden. Default: "YES". (since 3.0)
...
proc `scrollbar=`*(ih: Canvas_t, value: string) {.cdecl.} =
    ...
    ## RASTERSIZE. The SCROLLBAR and BORDER attributes affect the size
    ## of the drawing area. SCROLLBAR (creation only): Associates a
    ## horizontal and/or vertical scrollbar to the canvas. Default:
    ## "NO". The secondary attributes are all non inheritable.
...
proc `focus_cb=`*(ih: Canvas_t, cb: proc (ih: PIhandle, focus: cint): cint {.cdecl.}) {.cdecl.} =
    ## FOCUS_CB: Called when the canvas gets or looses the focus. It
    ## is called after the common callbacks GETFOCUS_CB and
    ## KILL_FOCUS_CB. int function(Ihandle \*ih, int focus); [in
