...
## IUP class: FlatButton
##
## NIUP type: FlatButton_t
##
## constructors
##
##    proc FlatButton(title: string): FlatButton_t
##
...
proc FlatButton*(title: string): FlatButton_t {.cdecl.} =
    ...
    ## IupButton, but since it is not a native control it has more
    ## flexibility for additional options. It can also behave like an
    ## IupToggle (without the checkmark). It inherits from IupCanvas.
...
proc `imageinactive=`*(ih: FlatButton_t, handle: Image_t | ImageRGB_t | ImageRGBA_t) {.cdecl.} =
    ...
    ## don't define TITLE nor IMAGE, but instead of BGCOLOR use
    ## FGCOLOR to set the color of the button.
...
proc `imagepress=`*(ih: FlatButton_t, handle: Image_t | ImageRGB_t | ImageRGBA_t) {.cdecl.} =
    ...
    ## IMPRESS is IMAGEPRESS, and so on). To define a button that only
    ## shows a color, do the same as in IupButton and don't define
    ## TITLE nor IMAGE, but instead of BGCOLOR use FGCOLOR to set the
    ## color of the button.
...
proc `value=`*(ih: FlatButton_t, value: string) {.cdecl.} =
    ...
    ## "OFF" or "TOGGLE". Default: "OFF". The TOGGLE option will
    ## invert the current state. Valid only when TOGGLE=Yes. Can only
    ## be set to ON for a toggle inside a radio, it will automatically
