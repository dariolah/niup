...
proc `alignment=`*(ih: FlatList_t, value: string) {.cdecl.} =
    ...
    ## obtained from the default value. Alignment does not includes
    ## the padding area.
...
proc `dragdroplist=`*(ih: FlatList_t, value: string) {.cdecl.} =
    ...
    ## list will be source and/or target. Default: NO.
...
proc `dropfilestarget=`*(ih: FlatList_t, value: string) {.cdecl.} =
    ## DROPFILESTARGET (non inheritable): Enable or disable the drop
    ## of files. Default: NO, but if DROPFILES_CB is defined when the
...
proc `iconspacing=`*(ih: FlatList_t, value: string) {.cdecl.} =
    ## ICONSPACING (non inheritable): spacing between the image and
    ## the text. Default: "2".
...
proc `padding=`*(ih: FlatList_t, value: string) {.cdecl.} =
    ...
    ## MARGIN attribute of the IupHbox and IupVbox containers, but
    ## uses a different name to avoid inheritance problems. Alignment
    ## does not includes the padding area. Default value: "2x2".
...
proc `removeitem=`*(ih: FlatList_t, value: string) {.cdecl.} =
    ...
    ## at 1. If value is NULL or "ALL" removes all the items.
...
proc `size=`*(ih: FlatList_t, value: string) {.cdecl.} =
    ...
    ## item, the default has room for 5 characters in 1 item. The
...
proc `spacing=`*(ih: FlatList_t, value: string) {.cdecl.} =
    ...
    ## IupList, it does not affects the internal margin. Not drawn
    ## with any item background color. Default: 0
...
proc `textpscolor=`*(ih: FlatList_t, value: string) {.cdecl.} =
    ## TEXTPSCOLOR: foreground color of a selected item. If not
    ## defined FORECOLORid will be used. (since 3.30)
...
proc `value=`*(ih: FlatList_t, value: string) {.cdecl.} =
    ...
    ## there is no selected item. For both cases, when setting NULL
    ## all items are deselected. The non changed items marked with 'x'
    ## are simulated internally by IUP in all systems. If you add or
    ## remove items to/from the list and you count on the 'x' values,
    ## then after adding/removing items set the VALUE attribute to
    ## ensure proper 'x' values.
...
proc `valuestring=`*(ih: FlatList_t, value: string) {.cdecl.} =
    ...
    ## MULTIPLE=NO. When set it will search for the first item with
    ## the same string.
...
proc `flat_action=`*(ih: FlatList_t, cb: proc (ih: PIhandle, arg2: cstring, arg3: cint, arg4: cint): cint {.cdecl.}) {.cdecl.} =
    ...
    ## \*text, int item, int state); [in C]ih:action(text: string,
    ## item, state: number) -> (ret: number) [in Lua]
