...
## IUP class: BackgroundBox
##
## NIUP type: BackgroundBox_t
##
## constructors
##
##    proc BackgroundBox(child: IUPhandle_t): BackgroundBox_t
##
...
proc `decorsize=`*(ih: BackgroundBox_t, value: string) {.cdecl.} =
    ## DECORSIZE (non inheritable): total size of the decoration in
    ## the format "WidthxHeight" (in C "%dx%d). Used only when
