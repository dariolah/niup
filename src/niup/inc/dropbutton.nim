...
## IUP class: DropButton
##
## NIUP type: DropButton_t
##
## constructors
##
##    proc DropButton(dropchild: IUPhandle_t): DropButton_t
##

proc DropButton*(dropchild: IUPhandle_t): DropButton_t {.cdecl.} =
    ## Creates an interface element that is a button with a drop down
    ## arrow. It can function as a button and as a dropdown. Its
    ## visual presentation can contain a text and/or an image. When
    ## dropped displays a child inside a dialog with no decorations,
    ## so it can simulate the initial function of a dropdown list, but
    ## it can display any layout of IUP elements inside the dropped
    ## dialog. When the user click outside the dialog, it is
    ## automatically closed. It inherits from IupCanvas.
...
proc `arrowimage=`*(ih: DropButton_t, handle: Image_t | ImageRGB_t | ImageRGBA_t) {.cdecl.} =
    ## ARROWIMAGE (non inheritable): Arrow image name. Use
...
proc `padding=`*(ih: DropButton_t, value: string) {.cdecl.} =
    ...
    ## the padding area. Default value: "3x3". Value can be
    ...
    ## and plus the horizontal space occupied by the arrow.
